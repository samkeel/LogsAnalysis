...
def popular_articles():
  ...
  query = "SELECT articles.title, COUNT(*) AS views " \
       "FROM articles INNER JOIN log ON log.path LIKE concat('%', articles.slug, '%') " \
...
def down_time():
    ...
    query = "select * from (select to_char(time, 'MONTH FMDD, YYYY') as dt, " \
         "ROUND((COUNT(*) filter (WHERE status like '%404%') * 100)::numeric /" \
         "count(*) filter (where status like '%200%'), 2) as error_rate from log " \
         "group by to_char(time, 'MONTH FMDD, YYYY')) ss where error_rate >= 1;"
